...
import com.medtech.model.componente.bateria.Bateria;
...
public class ComponenteDAO {
    ...
    private void inserirHardware(Connection conexao, String idComputador) throws SQLException {
        ...
        double bateriaTotal = bateria.exibeBateria();

        inserirHardwareDAO.inserirHardwareEspecifico(conexao, idComputador, "Memoria RAM", "Capacidade", memoriaTotal, "Memoria RAM");
        inserirHardwareDAO.inserirHardwareEspecifico(conexao, idComputador, "Armazenamento", "Capacidade", volumeTotal, "Dispositivo de Armazenamento");
        inserirHardwareDAO.inserirHardwareEspecifico(conexao, idComputador, "Bateria", "Capacidade", bateriaTotal, "Bateria");
        inserirHardwareDAO.inserirHardwareEspecifico(conexao, idComputador, "CPU", "Frequencia", cpuFreqTotal, "Processador");
    ...
    public void inserirUsoBateria(Bateria bateria, String nomeUsuario) throws SQLException {
        inserirComputadorSeNecessarioEmAmbos(cpu01.getIdCPU(), nomeUsuario);
        try (Connection conexao = obterConexaoMysql()) {
            int fkBateria = obterOuInserirBateria(conexao, cpu01.getIdCPU());
            inserirRegistroEmAmbos(bateria.exibeBateria(), cpu01.getIdCPU(), fkBateria);
        }
    }

    private int obterOuInserirBateria(Connection conexao, String idComputador) throws SQLException {
        int fkBateria = -1;
        try {
            fkBateria = obterFkHardware(conexao, "Bateria");
        } catch (SQLException e) {
            // Se não encontrar a bateria, insere ela antes de obter o ID
            inserirHardwareBateria(conexao, idComputador);
            fkBateria = obterFkHardware(conexao, "Bateria");
        }
        return fkBateria;
    }

    private void inserirHardwareBateria(Connection conexao, String idComputador) throws SQLException {
        // Aqui você insere os dados específicos da bateria
        // No seu caso, você já tem os dados da bateria, então vamos apenas inserir o hardware
        String descricaoHardware = "System Battery"; // Descrição da Bateria
        inserirHardwareDAO.inserirHardwareEspecifico(conexao, idComputador, "Bateria", "Capacidade", 75.0, descricaoHardware);
        // Você pode adicionar mais detalhes da bateria conforme necessário
    }
